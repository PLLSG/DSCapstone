---
#title: "DScapstone - Build NLM - 2- & 3-GRAMS"
#author: "PLLSG"
#date: "Nov 2017"
---
...
ngm2<-tokens(as.character(nstnc),
             what="word",ngrams=2,concatenator="_",
             remove_numbers=T,remove_punct=T,remove_symbols=T,
             remove_separators=T,remove_hyphens=T,
             remove_twitter=T,remove_url=T)

ngm3<-tokens(as.character(nstnc),
             what="word",ngrams=3,concatenator="_",
             remove_numbers=T,remove_punct=T,remove_symbols=T,
             remove_separators=T,remove_hyphens=T,
             remove_twitter=T,remove_url=T)
...
ngmfq2<-as.data.table(textstat_frequency(txtdfm2))
ngmfq3<-as.data.table(textstat_frequency(txtdfm3))

ngmfq<-merge(ngmfq2[,1:2],ngmfq3[,1:2],all=TRUE)
if(sum((itn/20)==1:40)) {
...
# Remove ngrams that have less than 6 occurrences
if(sum((itn/190)==1:2)) {
    nlm<-subset(nlm,frequency>=6)}
...
# Remove ngrams that have 1-5 occurrences
nlm<-subset(nlm,frequency>=6)


# Create 1- & 2-grams as key column for 2- & 3-ngrams
nlm$nkey<-word(nlm$feature,1,-2,sep="_")

# Set up key & order, save NLM
setkey(nlm,nkey)
setorderv(nlm,c("nkey","frequency","feature"),c(1,-1,1))
write_rds(nlm,"./course_data/final/en_US/nlm.rds")
